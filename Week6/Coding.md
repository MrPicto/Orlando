...
void loop() {
  ...
  Serial.println(sensorValue1);
    ...
    for (int thisNote1 = 0; thisNote1=36;) {         //用一個 for loop ，來連續演奏程式上面所定義的音符。46 為要演奏音符的總數目(含休止符)。
    int noteDuration = 1000 / noteDurations[thisNote1];
    tone(8, melody[thisNote1], noteDuration);                    //喇叭＋極接 Arduino pin 8, melody[thisNote] 為要演奏的那個音符的頻率，noteDuration 為要演奏的那個音符的節拍數。
